...
def run_scaling_experiment(param_name, param_values, default_I=40, default_N_device=150, default_N_edge=80):
    """
    运行参数扩展实验
...
def plot_latency_comparison(all_results):
    """绘制时延对比曲线（独立图例+坐标控制）"""
    strategies = ['cloud-only', 'edge-device', 'cloud-device','cloud-edge','proposed CEDC']
    ...
    param_config = [
        ('N_device', 'Device Resources', [20, 30, 40, 50, 60, 70]),
    ]
...
if __name__ == "__main__":
    ...
    # 获取目标数据
    target_data = all_results['N_device'][target_N_device]
