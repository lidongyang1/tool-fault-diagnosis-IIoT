...
class TaskScheduler:
    def __init__(self, I=200, N_device=10, N_edge=500):
        """初始化系统参数"""
        self.I = I  # 总任务数
...
def run_scaling_experiment(param_name, param_values, default_I=40, default_N_device=40, default_N_edge=80):

    results = {}
    strategies = ['Proposed GA', 'edge-only', 'cloud-only', 'edge-device', 'cloud-device','cloud-edge']
    ...
    for value in param_values:
        ...
        # 运行基准策略
        baseline_results = scheduler.benchmark_strategies(['edge-only',
...
def plot_latency_comparison(all_results):
    """绘制时延对比曲线（独立图例+坐标控制）"""
    strategies = [
                  'cloud-only', 'edge-device', 'cloud-device','cloud-edge','proposed CEDC']
    ...
    param_config = [
        ('N_edge', 'Edge Resources', [20,40, 60,80, 100, 120,140])
    ]
    for idx, (param, title, values) in enumerate(param_config, 1):
        ...
        # 坐标轴设置
        # ax.set_title(f'Latency vs {title}', pad=15)
...
if __name__ == "__main__":
    ...
    # 获取目标数据
    target_data = all_results['N_edge'][target_N_edge]
    cdec_latency = target_data['proposed CEDC']['total_latency']

    # 打印详细数据
    strategies = ['proposed CEDC', 'cloud-only', 'edge-device', 'cloud-device','cloud-edge']
    for strategy in strategies:
    ...
    # 计算优化百分比
    print("\n=== 时延优化百分比 ===")
    baseline_strategies = ['cloud-only', 'edge-device', 'cloud-device','cloud-edge']
    ...
    plot_latency_comparison(all_results)# 运行不同参数的扩展实验
